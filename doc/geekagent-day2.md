...
1. 询问“现在几点”时，模型返回 `get_current_time` 的 `tool_call`，程序执行工具，并用同一个 `tool_call_id` 把真实时间放回 history；
...
| | 应用代码匹配关键词 | 模型 function calling |
|---|---|---|
| 谁选择工具 | 我们写 `if/else` | 模型根据工具描述选择 |
| 支持新说法 | 要继续补规则 | 由模型理解自然语言 |
| 参数生成 | 应用代码自己解析 | 模型按 JSON Schema 生成 |
| 程序职责 | 理解问题并执行工具 | 声明工具、执行、回传结果 |

这里的 function calling 只是一套**调用协议**：模型不会真的执行函数，它只返回工具名和参数。真正接触时间、文件或网络的仍然是我们的程序，这也是工具调用循环不可省掉的原因。
