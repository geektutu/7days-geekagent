...
1. `.geekagent/GeekAgent.json` 为每个工具声明 `allow` / `ask` / `deny`，文件不存在时自动写入默认策略
2. 未配置的新工具默认 `deny`，配置写错时启动失败，不静默放宽权限
3. `ls` / `read` / `glob` / `write` / `patch` 只能访问 `root` 内部，指向根目录外的相对路径、绝对路径和符号链接都会被拒绝
4. 工具输出与错误信息中，名字含 `KEY` / `TOKEN` / `SECRET` / `PASSWORD` 的环境变量值会被屏蔽
5. Day 7 的 TUI、会话和用量面板继续工作，面板新增当前根目录；路径太长时优先显示最后一级目录
6. `write` 和 `patch` 在真正改文件前，把旧状态写入 `.geekagent/undo.json`
7. `/undo` 能恢复被覆盖的文件，也能删除刚创建的文件
8. 撤销记录在重启后仍然可用，成功恢复后立即消费
...
读取旧文件时只把 `ENOENT` 当成新建。权限不足等其他错误继续抛出，避免把一个读不到的已有文件误记成“不存在”。
