...
- `/new work` 创建空会话，`/open <id>` 来回切换后，每条会话仍保留自己的消息，`/sessions` 能看到当前会话与消息数；
- `/save` 把全部会话及当前 ID 写入 `.geekagent/sessions.json`，`/load` 恢复后仍停在保存时的会话；
- 正常退出会自动保存；临时的 `default` 会话会获得可再次打开的 ID，重启后可以继续此前对话。
...
| | 每个会话一个 `Chat` | 一个 `Chat` + `Sessions` |
|---|---|---|
| OpenAI client | 每个对象各持一份 | 全部会话共用 |
| 模型与工具配置 | 容易重复 | 只维护一份 |
| 会话差异 | 整个对象 | 只有 `history` |
| 切换方式 | 更换 `Chat` 对象 | 导出、导入消息数组 |

如果不同会话需要不同模型或工具，多个 `Chat` 会更自然；当前所有会话配置相同，所以只分离真正变化的 `history`。
