...
前四天已经接通对话、工具和文件操作，但它们都依赖 `Chat` 中的 `history`。这条数组会随着用户消息、模型回答和工具结果不断增长：
...
| 方式 | 优点 | 代价 |
|---|---|---|
| 全部保留 | 信息最完整 | 最终超过上下文窗口，成本和延迟持续增加 |
| 只留最近 N 条 | 实现简单，长度稳定 | 旧决定和关键线索直接丢失 |
| 摘要旧消息 | 用较少文字保留主要信息 | 摘要本身会丢细节，还要多调用一次模型 |

Day 5 选择第三种：复用现有模型生成摘要，不增加新的依赖和调用通道。接下来只需回答三个问题：什么时候压、压哪些消息、摘要怎样放回 `history`。

成熟系统还会组合滑动窗口、摘要、重要消息保留和外部记忆检索。当前 demo 只做滚动摘要，目的是先看清“旧消息变成一条摘要，再放回上下文”的核心过程。

## 1. 目标：自动压缩 + 手动入口

验收标准：

1. `history` 字符数超过阈值后，下一轮正式回答前先调用模型，把较早的多条消息替换成一条“此前对话摘要”；
2. 最近 6 条消息保留原文；压缩完成后继续追问早期事实，模型仍能从摘要中找回关键内容；
3. `/compact` 可以不等阈值手动执行同一逻辑，并显示本次合并了多少条旧消息。
...
最近几轮对话直接影响当前回答，不能一起压缩。因此我们把**最近 `KEEP_RECENT`（取 6）条以外**的消息视为旧消息，交给模型生成一条摘要；最近 6 条保留原文，再与摘要一起组成新的 `history`。6 条大约覆盖最近两三轮问答和相关工具结果。
...
- **旧事实仍然可用**：项目名、语言、作者和启动命令来自已压缩的旧消息，目标和代码目录来自保留的最近消息。模型能同时回答，说明摘要保留了前一部分的关键信息。
...
历史变短后仍然只存在于当前进程，而且不同话题还共用一条数组。下一步让对话彼此分开，并在程序退出后继续保留。
