...
问题已经很清楚：读取和写入共用 `run_shell`，权限粒度太粗。我们把读取做成不含写能力的专用工具；写入也使用专用工具，并在落盘前展示 diff。

| | 通用 `run_shell` | 专用文件工具 |
|---|---|---|
| 能力范围 | 一条命令几乎什么都能做 | 每个工具只做一种操作 |
| 读文件 | 也要确认整条命令 | `read` 可直接放行 |
| 写文件 | 命令执行前看不到具体变化 | `write` / `patch` 先展示 diff |
| 参数 | 一段 shell 字符串 | 路径、内容等结构化字段 |

主流 Coding Agent 通常同时保留两类工具：专用工具负责高频、可控的文件操作，shell 负责测试、构建等通用任务。这里不是抛弃 shell，而是把读写从它过宽的能力中拆出来。

## 1. 目标：五把文件工具 + 一个注册表

验收标准：

1. `ls` 列目录、`glob` 按模式找文件、`read` 读取文本；大文件可以用 `offset` 接着读，这三种操作不需要确认；
2. `write` 负责整体写入，`patch` 用唯一的旧文本定位局部修改；两者都先展示 diff，确认后才落盘；
3. 新工具只需调用 `registerTool` 注册，`toOpenAITools` 与 `execTool` 分别从同一张表生成模型清单和执行实现。
...
写入分成两个工具：`write` 接收 `path` + `content`，负责新建文件或整体替换；`patch` 接收 `hunks` 数组，用 `{ old, new }` 定位局部替换。两者分开后，模型修改一行时不必重传完整文件，确认界面也能只展示相关变化。
...
- 模型可以直接调用 `write`，收到结果后继续调用 `patch`，最后再统一回答。
- 当前终端不渲染 Markdown，因此模型返回的 `**` 和代码围栏会原样显示。
...
- 最后同样交给 `commitWrite`，统一完成 diff、确认和落盘。
...
现在模型能用专用工具完成“读取、修改、验证”。随着文件内容和工具结果不断进入 `history`，下一步要控制上下文长度，同时尽量保留早期对话中的关键信息。
