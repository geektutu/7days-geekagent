---
title: 从零实现 GeekAgent —— Day7 轻量 TUI 与用量面板
description: >-
  7天从零实现Agent教程，用 TypeScript 动手写一个最简单的 Agent/Harness。Day 7 给终端换一张桌：程序暂时接管整个终端，左侧放消息流，右侧常驻一块用量面板——本轮/累计 tokens 与上下文占用比例一边聊一边看得见，退出后原终端内容恢复。
...
> 前六天的终端只会不断追加日志，本轮用了多少 token、上下文还剩多少空间都看不到。今天让程序接管终端：左侧保留消息流，右侧常驻用量面板，持续显示本轮 / 累计 tokens 与上下文占用。退出后恢复原来的终端内容。
...
**第二样是上下文占用。** Day 5 会在 history 超过阈值后自动压缩。如果面板持续显示占用比例，我们也可以在接近窗口上限时主动执行 `/compact`。

这两类数据都需要固定显示，普通日志流做不到。因此我们使用全屏 TUI，把消息区、输入区和状态区分开；后续新增展示型状态时，也可以继续放进右侧面板。

## 1. 目标：一块桌 + 一块面板

验收标准：

- TUI 左侧持续显示用户、模型和工具消息，右侧面板在消息滚动时仍固定可见；窄终端会优先保留正文与输入区；
- 面板显示当前模型、会话、上下文估算、本轮 tokens 和累计 tokens；流式期间先估算，接口返回 usage 后换成真实值；
- 输入、流式回答、工具的 `[y/N]` 确认和已有会话命令都在同一界面完成，退出后恢复原终端。

**当天代码行数**：相对 Day 6，源码净增 295 行；新增 `tui.ts`（216 行）。
...
| | 手写 ANSI TUI | 使用 pi-tui |
|---|---|---|
| 屏幕刷新 | 自己计算光标与重绘区域 | 组件负责差量重绘 |
| 输入编辑 | 自己处理按键、中文宽度 | `Input` 统一处理 |
| 布局 | 手算行列位置 | `HStack` / `VStack` 组合 |
| 依赖与控制 | 零依赖、完全可控 | 增加依赖，少写大量界面底层代码 |
...
`reportUsage` 只把接口返回的数字交给外部回调，不改变工具循环和历史压缩流程。

### 3.3 index.ts：接入已有命令与状态
...
7. 执行 `npm run dev -- day7/index.ts < /dev/null`，确认非交互环境会提示需要真实终端。
...
界面已经能持续展示状态，但工具能否执行仍由代码写死，文件路径也没有统一边界。下一步把这些规则从工具实现中分离出来，并为误写提供恢复手段。
